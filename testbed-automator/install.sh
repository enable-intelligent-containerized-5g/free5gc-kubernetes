...
install-containerd() {
	if [ -x "$(command -v containerd)" ]; then
		...
		sudo apt-get update
		sudo apt-get -y  install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin		sudo mkdir -p /etc/containerd
...
autocompletion() {
	cecho "GREEN" "Adding completion and k alias to kubectl comand..."
